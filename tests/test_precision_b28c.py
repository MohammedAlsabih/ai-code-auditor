...
def test_a_literal_only_concat_is_constant_sql():
    # the sample AttendanceAbsenceService shape: parameterized SQL split over
...
def test_a_interpolation_of_ef_model_metadata_is_trusted():
    # the sample RowLevelSecurity shape
...
def test_a_local_helper_with_literal_call_sites_is_trusted():
    # the sample Reporting `Scoped` shape
...
def test_a_ef_placeholders_with_separate_args_are_constant():
    # the sample InvoiceNumbering shape: {0}/{1} placeholders live inside a RAW
...
def test_e_mangled_pem_is_not_a_secret():
    # the sample JwtRs256Tests shape: literal \n and a garbage body
...
def test_close5_chained_call_after_raw_sql_is_not_the_sql_api():
    ...
    # its receiver text mentions one (that judged `ct` as SQL). The sample
