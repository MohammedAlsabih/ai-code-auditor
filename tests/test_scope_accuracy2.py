...
def test_sample_shape_tests_project_inherits_from_sdk_web_api(tmp_path):
    """The literal Sample case: Tests -> Api(Sdk.Web) => AspNetCore provided."""
    _proj(tmp_path, "backend/Sample.Api", "", sdk="Microsoft.NET.Sdk.Web")
    tests = _proj(tmp_path, "backend/Sample.Api.Tests",
                  _projref("../Sample.Api/Sample.Api.csproj"))
    ad = _dotnet()
    ad.set_repo_root(tmp_path)
